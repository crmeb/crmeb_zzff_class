...
//        return $this->fetch('public/common_form');
        $field = [
            ...
            Form::number('vip_price', '产品会员售价')->min(0)->col(8),
...
//        if(!$data['keyword']) return Json::fail('请输入产品关键字');
        ...
        if ($data['vip_price'] == '' || $data['vip_price'] < 0) return Json::fail('请输入产品会员售价');
    ...
    public function edit($id)
    {
        ...
        $form = Form::create(Url::build('update', array('id' => $id)), [
            Form::select('cate_id', '分类', $product->getData('cate_id'))->setOptions(function () use ($mer_id) {
                $list = CategoryModel::getTierList(CategoryModel::where('mer_id', $mer_id));
                $menus = [];
                foreach ($list as $menu) {
            ...
            Form::input('store_name', '名称', $product->getData('store_name')),
            Form::input('store_info', '简介', $product->getData('store_info'))->type('textarea'),
            Form::input('keyword', '关键字', $product->getData('keyword'))->placeholder('多个用英文状态下的逗号隔开'),
            Form::input('unit_name', '单位', $product->getData('unit_name')),
            Form::frameImageOne('image', '封面图', Url::build('admin/widget.images/index', array('fodder' => 'image')), $product->getData('image'))->icon('image')->width('100%')->height('550px'),
            Form::frameImages('slider_image', '产品轮播图(640*640px)', Url::build('admin/widget.images/index', array('fodder' => 'slider_image')), json_decode($product->getData('slider_image')))->maxLength(5)->icon('images')->width('100%')->height('550px'),
            Form::number('price', '售价', $product->getData('price'))->min(0)->precision(2)->col(8),
            ...
            Form::number('cost', '成本价', $product->getData('cost'))->min(0)->col(8),
