...
class StoreCategory extends AuthController
{
    ...
    public function create()
    {
        $form = Form::create(Url::build('save'), [
            ...
            Form::frameImageOne('pic', '分类图标', Url::build('admin/widget.images/index', array('fodder' => 'pic')))->icon('image')->width('100%')->height('80%'),
            Form::number('sort', '排序'),
...
//                $list = (Util::sortListTier(CategoryModel::where('id','<>',$id)->select()->toArray(),'顶级','pid','cate_name'));
            ...
            Form::frameImageOne('pic', '分类图标', Url::build('admin/widget.images/index', array('fodder' => 'pic')), $c->getData('pic'))->icon('image')->width('100%')->height('80%'),
            Form::number('sort', '排序', $c->getData('sort')),
            Form::radio('is_show', '状态', $c->getData('is_show'))->options([['label' => '显示', 'value' => 1], ['label' => '隐藏', 'value' => 0]])
        ])->setMethod('post')->setTitle('添加分类');
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
